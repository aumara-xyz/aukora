...
| --- | --- | --- | --- |
| Reactive, receipt-chained, growing memory with governed forgetting (content-free chain) | `apps/brain/src`, `apps/brain/convex` | `apps/brain/test` (26) | Convex backend is the **headless simulated** `convex-test` (in-process), **not** a live cloud deploy |
| Provider-neutral brain attachment incl. a Nebius provider path | `apps/brain/src/nebiusProvider.ts` | `apps/brain/test/nebiusProvider.test.ts` | **Bounded and parked**: no live model calls, no weights, no endpoint/job IDs |
| Governed inward-out recursion: propose → ground → sandbox-rehearse → advisory review → refuse(stale/secret/authority) → hybrid AUMLOK owner-gate → sandbox-only apply → receipt/lineage | `apps/seed/src` | `apps/seed/test` (54) | Council review is **mock/deterministic**; apply is **sandbox-only** and never mutates a live repo |
...
Test totals: 145 (root) + 14 (kernel) + 4 (memory) + 26 (brain) + 54 (seed) + 21 (console) =
**264**, none borrowed from any external product suite.
