...
Test totals: **1528 passing** (+ **2 gated skips**, reported separately) across the full workspace suite — root, kernel, kernel-node, evidence, immune, memory, mind, council, council-node, brain, seed, spatial, supervisor, and the console fixture-regeneration guard — none borrowed from any external product suite. This total is MEASURED by `scripts/measure-test-totals.mjs` (a real `npm run test:all` run), written to `docs/generated/test-totals.json`, and its equality with this line **and** the README badge is enforced by `scripts/verify-test-totals.mjs` inside `test:all`; a stale hand-written number fails the gate. (`scripts/verify-continuity.mjs` guards the 191-item continuity ledger ↔ Atlas ↔ anatomy ↔ current-objects set — NOT the test totals; the earlier claim that it "keeps this total honest" was incorrect and is corrected here.)
