...
| --- | --- | --- | --- |
| Reactive, receipt-chained, growing memory with governed forgetting (content-free chain); corrupt-store fail-closed + reproducible runtime manifest | `apps/brain/src`, `apps/brain/convex` | `apps/brain/test` (39) | Convex backend is the **headless simulated** `convex-test` (in-process), **not** a live cloud deploy |
...
| Governed inward-out recursion: propose → ground → sandbox-rehearse → advisory review → refuse(stale/secret/authority) → hybrid AUMLOK owner-gate → sandbox-only apply → receipt/lineage; AURA trace law + receipt-before-row | `apps/seed/src` | `apps/seed/test` (74) | Council review is **mock/deterministic**; apply is **sandbox-only** and never mutates a live repo |
...
Test totals: 145 (root) + 14 (kernel) + 4 (memory) + 39 (brain) + 74 (seed) + 21 (console) =
**297**, none borrowed from any external product suite.
