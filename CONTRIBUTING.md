...
`test:all` runs provenance, the canonical boundary, the package suites, the full kernel gate
(typecheck, tests, build, compatibility manifest, SBOM, runtime matrix, package tarball), and the
organism suites (`memory`, `brain`, `seed`, `console`).
